...
\subsubsection{Chain Order Optimisation}

\begin{figure}[h]
  \centering
    \includegraphics[width=0.45\textwidth]{figures/chain.png}
  \caption{Runtime vs number of partitions.}
  \label{fig:chain}
\end{figure}

\cref{fig:chain} shows the runtime for increasing matrix densities with the optimal and non-optimal chain ordering. The matrices used in this experiment are of the following size:
\begin{itemize}
    \item $A: 5000\times 10$
    \item $B: 10\times 5000$
    \item $C: 5000\times 10$
...
The reason for this difference is that the optimal order first computes $B\times C$, producing a small intermediate of $10\times 10$, which can be efficiently used in the second multiply. Conversely, the non-optimal order has an intermediate of $5000\times 5000$, which quickly becomes very dense as the input density rises, leading to a huge shuffle volume and slow second multiplication.
...
The transition matrix $M=D^{-1}A$ was constructed by normalising rows by number of outgoing edges (outdegree).

Each page rank iteration followed

\[ r_{t+1}=\alphaM^Tr_t+(1-\alpha)1/N\]
where $\alpha=0.85$

Dangling nodes were handled directly in the engine to maintain mass conservation $(\Sigma  r\approx1)$.
...
The experiment showed that the algorithm converged within 10 iterations, with the total rank mass $\approx 1.00$, confirming numerical stability. Top nodes reached rank values of about 0.02.
